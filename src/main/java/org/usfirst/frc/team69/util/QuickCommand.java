...
public class QuickCommand {
     ...
     * @return The newly created {@link Command}
     */
    public static Command runUntil(Subsystem req, Runnable body,
    ...
    /**
     * Create a command that will run if the runnable condition is met, and if
     * so until the finished condition is met.
     * 
     * @param req
     *            The {@link Subsystem} that this command requires.
     * @param body
     ...
     * @return The newly created {@link Command}
     */
    public static Command runIf(Subsystem req, Runnable body,
                                BooleanSupplier finished,
     ...
     * {@link CommandGroup#addSequential(Command)} and
     ...
     * subsystem. The pid controller is enabled when the command starts and
     * disabled when it ends or is interrupted.
     * 
     * @param req
     ...
     * @param hold
     *            Whether or not to continue running the PID loop after the
     *            target setpoint is reached
     * @return The newly created {@link Command}
     */
    public static Command pidMove(Subsystem req, PIDController pid,
                                  double setPoint, boolean hold) {
        return new Command() {
            {
     ...
     * current input is. This is useful in combination with a manual control
     * system, where you want to move the system manually but hold it in place
     * using PID.
     * 
     * @param req
     *            The Subsystem to require
     ...
     * @param source
     *            The PIDSource (encoder, potentiometer, etc.) from which to
     *            take the input
     * @return The newly created command
     */
    public static Command pidHold(Subsystem req, PIDController pid,
                                  PIDSource source) {
        return new Command() {
            {
                if (req != null)
    ...
    /**
     * Constructs a command that moves a {@link PIDController} to be under a
     * given target, only if the current position is over that target.
     * 
     * @param req
     *            The subsystem to require
     * @param pid
     ...
     * @return The newly created {@link Command}
     */
    public static Command pidMoveUnder(Subsystem req, PIDController pid,
                                       PIDSource source, double target,
                                       boolean hold) {
        return new Command() {
            {
                if (req != null) // not sure when we're going to use pid outside
    ...
    /**
     * Constructs a command that moves a {@link PIDController} to be over a
     * given target, only if the current position is under that target.
     * 
     * @param req
     *            The subsystem to require
     * @param pid
     ...
     * @return The newly created {@link Command}
     */
    public static Command pidMoveOver(Subsystem req, PIDController pid,
                                      PIDSource source, double target,
                                      boolean hold) {
        return new Command() {
            {
     ...
     * subsystem, and end instantly. This has the effect of stopping any command
     * using the given subsystem, and of starting the default command, if one
     * exists.
     * 
     * @param subsystem
