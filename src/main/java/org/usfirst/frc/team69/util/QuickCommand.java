...
public class QuickCommand {
     ...
     * @param body The {@link Runnable} that will run once.
     ...
     * 
     * @param req The {@link Subsystem} that this command requires.
     * @param body The {@link Runnable} that will run repeatedly.
     * @return The newly created {@link Command}
     */
     ...
     * @param body The {@link Runnable} that will run repeatedly.
     * @param finished The condition that signals the end of the command.
     * @param runnable The condition that signals the command should be run
     * @return The newly created {@link Command}
     */	
    ...
    /**
     * Constructs a command that uses a {@link PIDController} to control a subsystem.  The pid controller
     * is enabled when the command starts and disabled when it ends or is interrupted.
     * 
     ...
     * @param hold Whether or not to continue running the PID loop after the target setpoint is reached
     * @return The newly created {@link Command}
     */
    public static Command pidMove(Subsystem req, PIDController pid, double setPoint, boolean hold) {
        return new Command() {
            {
                if (req != null) //not sure when we're going to use pid outside a subsystem lol
                    requires(req);
            }
    ...
    /**
     * Constructs a Command that holds a {@link PIDController} at whatever the current input is.  This is
     * useful in combination with a manual control system, where you want to move the system manually
     * but hold it in place using PID.
     * 
     * @param req The Subsystem to require
     * @param pid The PIDController to use
     * @param source The PIDSource (encoder, potentiometer, etc.) from which to take the input
    ...
    /**
     * Constructs a command that moves a {@link PIDController} to be under a given target, only if the current position is over that target.  
     * 
     * @param req The subsystem to require
     * @param pid The PID controller to use
     * @param source The PIDSource (encoder, potentiometer, etc.) from which to take the input
     ...
     * @return The newly created {@link Command}
     */
    public static Command pidMoveUnder (Subsystem req, PIDController pid, PIDSource source, double target, boolean hold) {
        return new Command() {
            {
                if (req != null) //not sure when we're going to use pid outside a subsystem lol
                    requires(req);
    ...
    /**
     * Constructs a command that moves a {@link PIDController} to be over a given target, only if the current position is under that target.  
     * 
     * @param req The subsystem to require
     * @param pid The PID controller to use
     * @param source The PIDSource (encoder, potentiometer, etc.) from which to take the input
