...
/**
 * A class which represents arcade drive.  Arcade drive uses two values: move
 * and rotate.  Move determines forwards-and-backwards motion, while rotate
 * determines rotational motion.  This drive mode is useful for writing
 * autonomous methods which attempt to track a target, or hold a gyro angle.
 * It can also be used to drive directly from the joystick.
 * 
 * This class simply wraps {@link RobotDrive#arcadeDrive(double, double, boolean)}.
 * 
...
public class ArcadeDriveParams implements DriveParameters {
     ...
     * @param squareInputs Whether to square the inputs.  This is desirable
     * if the input is coming from a joystick, as it creates a "soft deadzone".
     * If coming from another source, like a PID controller, this should be
     * <code>false</code>.
     */
