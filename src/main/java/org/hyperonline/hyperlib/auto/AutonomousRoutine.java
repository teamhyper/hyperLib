...
/**
 * The {@link AutonomousRoutine} class contains all the information needed for a particular
 * autonomous routine. It contains the code to build the command, as well as information about what
 * preferences need to be set, and what other routines may be called as subroutines.
 *
 * <p>Logic for deciding which routine should be placed in the AutonomousStrategy class.
 *
 * @author James Hagborg
 */
public abstract class AutonomousRoutine implements NTSendable {
      ...
      if (field.isAnnotationPresent(AutoPref.class)
        ...
        try {
          ...
          DriverStation.reportError(
              "Could not initialize non-public field "
                  + field.getName()
                  + " of class "
                  + getClass().getName(),
              false);
  ...
  /**
   * Abstract method for implementing the actual logic of the command. Ideally, this would use the
   * preferences and subroutines that have already been declared.
   *
  ...
  /**
   * Indicate that another routine is a subroutine of this one. This does not affect the execution
   * of code, but is used to display preferences to the driver station. Adding two subroutines with
   * the same name has no effect.
   *
   * @param routine The subroutine to add.
  ...
  /**
   * Two AutonomousRoutines are considered to be equal if they have the same name, with the
   * exception that two unnamed routines are equivalent iff they occupy the same memory location.
   */
  @Override
