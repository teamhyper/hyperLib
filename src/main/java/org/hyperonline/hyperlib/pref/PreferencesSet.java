...
/**
 * This class provides a set of related preferences, with some functionality grouped together. Each
 * preference is prefixed by the same name, so that the preferences all show up near each other in
 * the dashboard.
 *
 * <p>Example use-cases are subsystems, PID controllers, and specific tasks like vision and driving,
 * which may have many related preferences.
 *
 * <p>It is safe to share this object, as well as the Preference objects it creates, among multiple
 * threads.
 *
...
public class PreferencesSet {
   ...
   * called.
   *
   * @param name The prefix to use for all preferences in the set. This name must be unique among
   *     all {@link PreferencesSet} objects.
  ...
  /**
   * Check the set name does not exist and add it if it does. This needs to be synchronized to
   * access the global list.
   *
   * @param name The name to add and check
   */
  private static synchronized void registerSetName(String name) {
